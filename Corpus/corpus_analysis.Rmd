...
options(scipen = 1, digits = 3, dplyr.summarise.inform = FALSE)
```
...
responses_by_age <- parent_responses %>%
  ...
  filter(!is.na(gloss), !is.na(age_years), plural_type == "error") %>%
  group_by(plural_type, age_years) %>%
  summarise(replay = sum(replay, na.rm=TRUE), reformulation = 
               sum(reform.true, na.rm=TRUE), other = sum(other.true, na.rm =TRUE)) %>%
  ungroup() %>%
  complete(plural_type, age_years, fill=list(n.children = 0, n.utterances = 0, replay = 0, reformulation = 0, other = 0))
...
When only including children up to and including 4 years of age, there are `r n_utterances_four` utterances across `r n_child` children.
